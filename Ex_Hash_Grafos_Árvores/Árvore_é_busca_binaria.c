...
{
    if (vazia(arvore)) return true;
    
    // Verifica se a subarvore esquerda e uma arvore binaria de busca
    if (!vazia(arvore->esq))
    {
        // Se o no da subarvore esquerda for maior que o no atual, a arvore nao e binaria de busca
    ...
    return eBinariaBusca(arvore->esq) && eBinariaBusca(arvore->dir);
}
